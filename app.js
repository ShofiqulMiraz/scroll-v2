...
window.onscroll = function () {
  ...
  const span = document.querySelector(".nav span");
  if (
    sectionsListEl[0].getBoundingClientRect().top <= 0 &&
    sectionsListEl[0].getBoundingClientRect().bottom >= 0
  ) {
    span.style.transform = `translateY(${perNav * 0}px)`;
    prev = 1;
  } else if (
    sectionsListEl[1].getBoundingClientRect().top <= 0 &&
    sectionsListEl[1].getBoundingClientRect().bottom >= 0
  ) {
    span.style.transform = `translateY(${perNav * 1}px)`;
    prev = 2;
  } else if (
    sectionsListEl[2].getBoundingClientRect().top <= 0 &&
    sectionsListEl[2].getBoundingClientRect().bottom >= 0
  ) {
